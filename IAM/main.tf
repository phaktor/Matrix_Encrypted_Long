resource "aws_iam_role" "SSM-MATRIX-ROLE" {
 name = "SSM-MATRIX-ROLE"
...
resource "aws_iam_policy_attachment" "SSM-Role-Attachment" {
  ...
  roles      = [aws_iam_role.SSM-MATRIX-ROLE.name]
  policy_arn = data.aws_iam_policy.SSM-Policy.arn
}

resource "aws_iam_instance_profile" "SSM-Instance-Profile" {
  name = "SSM-Matrix-Instance-Profile"
  role = aws_iam_role.SSM-MATRIX-ROLE.id
}